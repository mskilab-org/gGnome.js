class Interval extends Base {
  ...
  constructor(inter) {
    ...
    this.popoverTitle = this.getPopoverIntervalTitle();
    this.popoverContent = this.getPopoverIntervalContent();
  }

  // The title for the popover on the intervals
  getPopoverIntervalTitle() {
    return 'Interval #' + this.title;
  }

  // The content for the popover of the intervals
  getPopoverIntervalContent() {
    let content = '';
    [{label: 'Chromosome', value: this.chromosome}, {label: 'Jabba', value: this.y}, {label: 'Start Point', value: d3.format(',')(this.startPoint)},
     {label: 'End Point', value: d3.format(',')(this.endPoint)}, {label: 'Interval Length', value: d3.format(',')(this.intervalLength)}, {label: 'Strand', value: this.strand}].forEach(function(e,j) {
