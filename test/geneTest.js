...
// constructor
 ...
 describe('testing gene.js', function() {
  it('Gene constructor', function() {
    ...
    assert.equal(gene.margins['arrow'], 0); 
    assert.equal(gene.coefficient, 0.66);
  });
});
...
describe('testing gene.js', function() {
  it('Gene points()', function() {
    ...
    expect(gene_strand_positive.points[2]).to.be.NaN;
    expect(gene_strand_positive.points[3]).to.be.NaN;
    expect(gene_strand_positive.points[4]).to.be.NaN;
    assert.equal(gene_strand_positive.points[5],  0);
    expect(gene_strand_positive.points[6]).to.be.undefined;
    assert.equal(gene_strand_positive.points[7],  0);
    expect(gene_strand_positive.points[8]).to.be.NaN;
    assert.equal(gene_strand_positive.points[9],  0);
    expect(gene_strand_positive.points[10]).to.be.NaN;
    ...
    assert.equal(gene_strand_negative.points[5], 0);
    assert.equal(gene_strand_negative.points[6], 0);
    assert.equal(gene_strand_negative.points[7], 0);
    assert.equal(gene_strand_negative.points[8], 0);
    assert.equal(gene_strand_negative.points[9], 0);
